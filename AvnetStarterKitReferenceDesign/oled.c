...
void update_oled()
{
	switch (oled_state)
	{
		...
		case 1:
		{
			update_other(light_sensor,current ,voltage); 
		}
		break;
		case 2:
		{
...
void update_other(float x, float y, float z)
{
	...
	uint8_t str_tbd1[] = "Volts:";
	uint8_t str_tbd2[] = "Amps:";
	...
	// Draw the units of y
	sd1306_draw_string(sizeof(str_tbd1) * 6 + (get_str_size(string_data) + 1) * 6, OLED_LINE_2_Y, "Amps", FONT_SIZE_LINE, white_pixel);
	...
	// Draw the units of z
	sd1306_draw_string(sizeof(str_tbd2) * 6 + (get_str_size(string_data) + 1) * 6, OLED_LINE_3_Y, "Volts", FONT_SIZE_LINE, white_pixel);

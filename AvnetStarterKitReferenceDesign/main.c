...
static void SocketEventHandler(EventData *eventData)
{
	...
	for (int i = 4; i < 8; i++)
	{
		analog_data2.u8[i] = rxBuf[i];
	}
	for (int i = 8; i < 12; i++)
	{
		analog_data3.u8[i] = rxBuf[i];
	}
	...
	// We can simplify the factors, but for demostration purpose it's OK
	 ...
	 current = (((float)analog_data2.u32*2.5/4095)- 1.25)/100 ; // 100mV/A is sensitivity of the hall sensor 
     voltage =  ((float)analog_data3.u32*2.5/4095)/0.2; //dividing it with the divider value
